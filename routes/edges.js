...
// INDEX gets all the concepts
router.post("/query", async (req, res) => {
  try {
    // const { conceptString, maxEdges, minEdges } = req.body
    
    const results = await pool.query(SQL`
    select * from nodes n join edges e on e.target_id = n.id
    `, [])

    // const results = await pool.query(
    //   sql,
    //   [conceptString, maxEdges?maxEdges:999, minEdges?minEdges:1]
    // )
    // const concept = { id: results.rows[0]['concept_id'], label: results.rows[0]['concept_name'] }
...
router.post("/", async (req, res) => {
  try {
    ...
    const concept = await pool.query(
      "INSERT INTO dbo.concepts(name) VALUES ($1) RETURNING *;",
      [name]
    )
